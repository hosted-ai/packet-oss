...
import { verifyInvestorSessionToken } from "@/lib/investor";
import {
  getTwoFactorStatus,
  startTwoFactorSetup,
...
/**
 * GET /api/investor/two-factor
 * Get 2FA status for the authenticated investor
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Investor 2FA status error:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Investor 2FA action error:", error);
    return NextResponse.json(
